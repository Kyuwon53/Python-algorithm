...
- 입력으로 들어온 문자열은 두 글자씩 끊어서 다중집합의 원소로 만든다. 
  - 이때 영문자로 된 글자 쌍만 유효하고, 기타 공백이나 숫자, 특수 문자가 들어있는 경우는 그 글자 쌍을 버린다. 대소문자 구분을 하지 않기때문에 대문자로 치환하던 소문자로 치환한다. 
- 두 집합의 교집합을 구한다
- 두 집합의 합집합을 구한다
- 교집합 / 합집합 * 65536 의 정수부만 출력한다. 

### 💬 회고 
- 깊은 복사를 하지 않아서 사이드 이팩트때문에 해결에 시간이 걸렸다. (반성)
- 좀 더 변수명을 잘 쓸 수 있지 않았을까 

### 👉 [뉴스 클러스터링](https://programmers.co.kr/learn/courses/30/lessons/17677?language=python3)
